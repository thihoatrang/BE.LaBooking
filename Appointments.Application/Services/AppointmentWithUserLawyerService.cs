...
namespace Appointments.Application.Services
{
    public class AppointmentWithUserLawyerService
    {
        ...
        // Helper to safely parse the Services field into a list of strings
        private List<string> ParseServices(string? servicesRaw)
        {
            if (string.IsNullOrWhiteSpace(servicesRaw))
                return new List<string>();

            var raw = servicesRaw.Trim();
            ...
            // Handle naive ToString() of a List e.g. "List`1 [A, B]" or "List[A, B]"
            var start = raw.IndexOf('[');
            var end = raw.LastIndexOf(']');
            if (start >=0 && end > start)
            {
                var inner = raw.Substring(start +1, end - start -1);
                return inner.Split(separators, StringSplitOptions.RemoveEmptyEntries)
                    .Select(s => s.Trim())
                    .Where(s => !string.IsNullOrEmpty(s))
                    .ToList();
            }
        ...
        public async Task<IEnumerable<AppointmentWithUserLawyerDTO>> GetAllAppointmentsWithUserLawyerAsync()
        {
            ...
            foreach (var appointment in appointments)
            {
                ...
                var servicesList = ParseServices(appointment.Services);
                ...
                result.Add(new AppointmentWithUserLawyerDTO
                {
                    ...
                    ScheduledAt = appointment.ScheduledAt?.ToDateTime(TimeOnly.MinValue) ?? DateTime.MinValue,
                    Slot = appointment.Slot ?? string.Empty,
                    CreateAt = appointment.CreateAt?.ToDateTime(TimeOnly.MinValue) ?? DateTime.MinValue,
                    Status = (AppointmentStatus)appointment.Status,
                    IsDel = appointment.IsDel,
                    Note = appointment.Note,
                    Spec = appointment.Spec ?? string.Empty,
        ...
        public async Task<Appointment> CreateAppointmentAsync(Appointment appointment)
        {
            try
            {
                ...
                var newAppointment = new Appointment
                {
                    ...
                    Note = appointment.Note,
                    Status = appointment.Status == 0 ? (int)AppointmentStatus.Pending : appointment.Status,
                    IsDel = false,
                    CreateAt = DateOnly.FromDateTime(DateTime.Now),
                    Services = NormalizeServicesString(appointment.Services)
                };
        ...
        public async Task<AppointmentWithUserLawyerDTO?> UpdateAppointmentAsync(int id, Appointment updatedAppointment)
        {
            ...
            appointment.Services = NormalizeServicesString(updatedAppointment.Services);
